Shader "Unlit/Vfx_Particle_Alp"
{
    ...
    Properties
    {
        ...
        //[Enum(BlendMode)]_dst_blend_factory("目标混合因子",int)=10
        ...
        _tex("贴图", 2D) = "white" {}
        [Toggle(_UV_SPEED_CUSTOM)]_UV_SPEED_CUSTOM("粒子custom1.xy(uv0.zw)控制uv偏移",int) = 0
        ...
        v2f vert(a2v v){
          ...
          #if _USE_TEX
          o.color = v.color;
          o.uv_tex = TRANSFORM_TEX(v.uv.xy,_tex);
          o.uv_tex_mask = TRANSFORM_TEX(v.uv.xy,_tex_mask);
          #endif
          // noise
          #if _USE_UV_NOSIZE
          o.uv_uv_nosize = TRANSFORM_TEX(v.uv.xy,_uv_nosize_tex);
          #endif
          // dissove
          #if _USE_DISSOLVE
          float time = fmod(_Time.y, 2e5);
		  o.uv_dissiove = TRANSFORM_TEX(v.uv.xy, _diss_tex) + frac(_diss_tex_offset.xy * (time-_CustomTime));
          #endif
          return o;
        }
